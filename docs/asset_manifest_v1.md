...
|---|---|---|
| Player HD presentation | Implemented in issue #150; body source upgraded in issue #165: the accepted Flux-derived non-commercial `player_directional_atlas.png` is a 1024×256 four-cell directional sheet (256×256 cells with a 190 px content box: north/west/south/east at columns 0–3; authored side cells are never runtime-flipped), selected by `PlayerHdPresentation` from the live `PlayerVisual.facing_label` at a 42 px display-height contract. **Issue #224 repairs the locomotion and melee identity contract:** `player_locomotion_response_atlas.png` (1280×1024) and `player_melee_body_atlas.png` (768×1024) are byte-identical deterministic Pillow derivatives of that accepted canonical atlas, not independent procedural characters. The locomotion cells retain the hood, cloak, teal/magenta palette, proportion, and illustrated materials while using feather-blended planted lower-cloak/boot gait, passing, settle, and hurt poses; the melee cells retain that same actor through wind-up, committed contact, and cross-body recovery. They carry the same `flux-non-commercial-license` restriction as their canonical source; see `assets/sprites/LICENSES.md` for generator/provenance/MD5 records. The separately owned steel weapon has explicit front body layering and phase-resolved hand grips for all four cardinal directions, while the existing 0.12-second hit window, collision, damage, input, health, and `CombatFxSpawner` slash ownership remain unchanged. Dash/relic presentation remains as authored in issues #195/#193. | Gameplay state ownership, movement, health, invulnerability, respawn, collision, input, melee damage/hitbox/timing, projectile behavior, and combat-FX ownership. |

Issue #224 Web bundle evidence (Godot 4.7, same-machine release export):
`index.pck` is 9,310,608 B, +340,816 B (332.8 KiB) relative to the current-main
8,969,792 B baseline, and remains under the 12 MiB visual-bible guardrail;
`index.wasm` is unchanged at 39,509,339 B. The real browser acceptance route
checks all cardinal locomotion plus wind-up/contact/recovery at gameplay scale,
with the canonical body identity, planted gait readability, hand-held foreground
steel, and no duplicate/hidden player body required before merge.
